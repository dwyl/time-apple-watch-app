...
class TasksTableViewController: UITableViewController, WCSessionDelegate {
    ...
    func subscribeToNotifications() {
        ...
        notification.addObserver(self, selector: #selector(timerStartedOnPhone), name: NSNotification.Name(rawValue: "timerStartedOnPhone"), object: nil)
    }
    ...
    func timerStartedOnPhone(notification: Notification) {
        if let userInfo = notification.userInfo, let project_name = userInfo["project_name"] as? String, let start_time = userInfo["start_time"] {
            // send message to the watch and let it handle the timer etc on there.
            if let watchSession = session {
